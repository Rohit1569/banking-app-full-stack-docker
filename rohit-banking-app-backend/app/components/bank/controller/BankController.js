...
class BankController {
    ...
    async createBank(settingsConfig, req, res, next) {
        try {
            ...
            const newBank = await this.bankService.createBank(settingsConfig, bankName)
    ...
    async getAllBanksForUser(settingsConfig, req, res, next) {
        try {
            const logger = settingsConfig.logger;
            logger.info(`[BankController] : Inside getAllBanks`);

            const queryParams = req.query
            const { count, rows } = await this.bankService.getAllBanksForUser(settingsConfig, queryParams)
            res.set('X-Total-Count', count)
            res.status(StatusCodes.OK).json(rows)
            return
        } catch (error) {
